...
public class CheckImplicitDependenciesTask extends DefaultTask {
    ...
    public final void checkImplicitDependencies() {
        ...
        List<ResolvedArtifact> usedButUndeclared = necessaryArtifacts.stream()
                .filter(artifacts -> artifacts.stream().anyMatch(this::isArtifactFromCurrentProject))
                .filter(artifacts -> artifacts.stream().anyMatch(this::shouldIgnore))
                .filter(artifacts -> artifacts.stream().anyMatch(declaredArtifacts::contains))
